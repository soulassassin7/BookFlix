        ...
        {nowPlayingMovies.slice(0, 22).map((movie, index) => (
          ...
          >
            <div className="relative">
              {/* FIX 1: Access the image URL correctly */}
              <img
                src={movie.primaryImage.url} 
                alt="poster"
                className="w-full h-auto object-cover brightness-90 rounded-lg" // Changed h-full to h-auto for aspect ratio
              />
              <div className="text-sm flex items-center justify-between p-2 bg-black/70 w-full rounded-b-lg absolute bottom-0 left-0">
                
                {/* FIX 2: Removed the star rating and votes since they don't exist in our local data */}
                <div className="flex items-center gap-1 text-gray-400 text-xs">
                  {/* StarIcon and movie.averageRating removed */}
                </div>
                <p className="text-gray-400 text-xs">{/* thousandConvert(movie.numVotes) removed */}</p>
            ...
            {/* FIX 3: Access the title text correctly */}
            <p className="text-base font-semibold truncate mt-2">{movie.titleText.text}</p>
            {/* FIX 4: Removed the release date since it's not in the simplified data */}
            <p className="text-sm text-gray-400">{/* movie.releaseDate removed */}</p>
          </div>
        ))}
      </div>
