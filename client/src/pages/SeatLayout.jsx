...
const SeatLayout = () => {
  ...
  const getoccupiedSeats = async() => {
    try {
      // Corrected API route from /seats/ to /occupiedseats/
      const {data} = await axios.get(`/api/booking/occupiedseats/${selectedTime.showId}`);
      if(data.success) {
  ...
  // --- NEW LOGIC TO HANDLE "BACK BUTTON" SCENARIO ---
  useEffect(() => {
    const checkForUnpaid = async () => {
      // Run this check only after we have the user and show data
      if (user && show && show.datetime[date]) {
        // Create an array of all show IDs for the current day
        const showIds = show.datetime[date].map(item => item.showId);

        // Call our new backend endpoint to check for existing unpaid bookings
        const { data } = await axios.post('/api/booking/check-unpaid', { showIds }, {
          headers: { Authorization: `Bearer ${await getToken()}` }
        });

        if (data.success && data.hasUnpaidBooking) {
          // If we find one, alert the user and redirect them
          toast.error("You have an existing unpaid booking. Please complete it first.", { duration: 4000 });
          navigate('/my-bookings');
        }
      }
    };
    checkForUnpaid();
  }, [show, user, date, navigate, getToken, axios]); // Dependencies that trigger this check
  // --- END OF NEW LOGIC ---
  ...
  // The JSX below is exactly the same as your original file.
  return (
